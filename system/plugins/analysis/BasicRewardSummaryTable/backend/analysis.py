...
# Final function
class BasicRewardSummaryTable():
    def __init__(self, info):
        self.info = info
        # df, selectedVariables, selectedAssigners
    def analysis(self):
        result_df = None
        data = self.info['df']
        # filter out selected assigners
        if len(self.info['selectedAssigners']) > 0:
            data = self.info['df'][self.info['df']['assigner'].isin(self.info['selectedAssigners'])]
        all_combs = get_all_combinations(self.info['selectedVariables'])
        if len(self.info['selectedVariables']) == 0:
        ...
        result_df = result_df.sort_values(by= self.info['selectedVariables'] + ['treatment'], ascending=True)
        ...
        outcomes = [self.info['df'][self.info['df']['treatment'] == version]['outcome'].dropna() for version in unique_versions]
        f_value, p_value = stats.f_oneway(*outcomes)
        power = calculate_statistical_power(self.info['df'], 'outcome')
        for index, row in result_df.iterrows():
            if all(row[key] == "" for key in self.info['selectedVariables']):
                result_df.loc[index, 'P value'] = round(p_value,3)
                result_df.loc[index, 'Power'] = power
