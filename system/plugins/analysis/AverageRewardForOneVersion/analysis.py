...
class AverageRewardForOneVersion():
    def __init__(self, info):
        self.info = info

    def analysis(self):
        df = self.info['df']
        selectedVersion = self.info['selectedVersion']
        selectedPolicy = self.info['selectedPolicy']
        selectedVariable = self.info['selectedVariable']
        per_day = self.info['per_day']
        # Filter dataframe
        ...
        result_df, groups = json.loads(result_df.to_json(orient="records")), [c for c in result_df.columns if c != 'time']
